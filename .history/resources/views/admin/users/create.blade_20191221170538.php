                            ...
                            <form action="{{route('users.store')}}" method="POST"
                                ...
                                <div class="row">
                                    <div class="col-lg-6">
                                        ...
                                        <div class="form-group">
                                            <select name="authority"  id="authority_select" class="form-control input-md" required>
                                                <option value="">Please select authority</option>
                                                @foreach($authorities as $key=>$value)
                                                    <option value="{{$value->title}}"  @if(old('authority') == $value->title) selected @endif>{{ucwords($value->title)}}</option>
                                                @endforeach
                                                <option value="Other" @if(old('authority') == "Other") selected @endif>Other</option>
                                            </select>
                                        </div>
                                        <div class="form-group  authority_other display-none">
                                            <h5>New Authority:</h5>
                                            <input type="text" value="" placeholder="add other authority" class="form-control input-md" id="authority_other" name="authority_other"/>
                                        </div>
                                    </div>
                                    <div class="col-lg-6">
                                        <div class="form-group">
...
@section('scripts')
    ...
    <script>
        $(function () {
            $(document).on('change',"#authority_select", function(){
                if($(this).val()=="Other"){
                    console.log(`======val====${$(this).val()}`);
                    $(".authority_other").show();
                    $("#authority_other").val("");
                }else{
                    $(".authority_other").hide();
                    $("#authority_other").val("");
                }
            });
        });
